...
$router->post('/admin/tickets/filters/save', function () {
    ...
    // template decides which saved filter is the *active* one with a strict
    ...
    foreach (ticketBoolFilterKeys('admin') as $key) {
        if (!empty($_POST[$key])) {
            $filterData[$key] = '1';
        }
    }
    ...
    switch ($action) {
        ...
        case 'group':
            ...
            if ($groupId !== null) {
                ...
                $chk->execute([$groupId]);
                if (!$chk->fetchColumn()) { flash('error', 'Invalid group.'); redirect('/admin/tickets'); }
            }
