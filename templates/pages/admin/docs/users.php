...
<ul class="text-muted mb-2">
    ...
    <li><strong>At creation</strong> — the same block is on <a href="/admin/users/create">Add User</a>, so a new agent starts life in the right groups instead of needing a second trip. If you save a staff user with no group at all, a confirmation names the consequences and lets you go ahead anyway (some sites do run agents whose visibility comes only from a permission level or from <?= label('location.singular', 'location') ?> visibility).</li>
</ul>
