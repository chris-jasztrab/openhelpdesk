...
// "Require a resolution note before close" gate (per ticket type). The modal
// only fires when this type opts in, the CLOSER is the assigned owner, and no
// staff comment (public reply OR internal note) has been recorded yet. The live
// closed-status check happens in JS against these slugs.
...
<div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4 ticket-sticky-header" id="tour-ticket-header">
    <div>
...
<div class="row g-4">
    ...
    <div class="col-lg-6">
        ...
        <div class="card border-0 shadow-sm mb-4" id="similarTicketsCard"
             ...
             data-similar-url="/agent/tickets/<?= (int) $ticket['id'] ?>/similar"
    ...
    <div class="col-lg-3">
        ...
        <div class="card border-0 shadow-sm" id="tour-update-ticket">
            ...
            <div class="card-body">
                <form method="POST" action="/agent/tickets/<?= $ticket['id'] ?>/update" id="updateTicketForm"
...
/* ── Resolution-note-on-close gate (require_resolution_on_close types) ──
   ...
   or a logged reason to close without one. Both paths resubmit THIS form so any
   other pending changes (priority, assignee, …) are preserved. The reply
   "Send & change status" path needs no gate — it always carries a note. */
(function () {
    ...
    // Bootstrap's bundle may load after this inline script parses, so resolve the
    // modal instance lazily (by submit time it is always available).
    ...
    form.addEventListener('submit', function (e) {
        if (form._resolutionAck) return;      // already handled by a modal button
        if (!isClosing()) return;             // not a close → normal submit
        e.preventDefault();
        var m = getModal();
    ...
    // "Close without a note" reveals the reason picker; choosing a reason closes.
...
ClassicEditor.create(document.querySelector('#replyEditor'), {
    ...
    function showCollisionModal(collision, formEl) {
        ...
        review.href = '/agent/tickets/<?= (int) $ticket['id'] ?>';
