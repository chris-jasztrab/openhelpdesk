...
$actionIcons  = ['created' => 'bi-plus-circle text-success', 'assigned' => 'bi-person-check text-primary', 'status_changed' => 'bi-arrow-repeat text-warning', 'priority_changed' => 'bi-flag text-danger', 'comment' => 'bi-chat-dots text-info', 'internal_note' => 'bi-lock text-secondary', 'sla_set' => 'bi-stopwatch text-primary', 'sla_paused' => 'bi-pause-circle text-warning', 'sla_resumed' => 'bi-play-circle text-success', 'merged' => 'bi-arrow-right-circle text-secondary', 'split' => 'bi-scissors text-warning'];
$actionLabels = ['created' => 'Created', 'assigned' => 'Assigned', 'status_changed' => 'Status Changed', 'priority_changed' => 'Priority Changed', 'comment' => 'Comment', 'internal_note' => 'Internal Note', 'sla_set' => 'SLA Set', 'sla_paused' => 'SLA Paused', 'sla_resumed' => 'SLA Resumed', 'merged' => 'Merged', 'split' => 'Split'];
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="/agent/tickets/<?= (int) $ticket['id'] ?>/fields">
                    <?= csrfField() ?>
                    <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-check mt-1">
                            <input class="form-check-input" type="checkbox" name="<?= e($cfKey) ?>" value="1"
                                   id="agt_<?= e($cfKey) ?>" <?= $cfVal === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label" for="agt_<?= e($cfKey) ?>">Yes</label>
                        </div>
                        ...
                        ?>
                        <div class="d-flex flex-column gap-1" id="agt_dep_<?= (int) $cf['id'] ?>">
                            <select class="form-select form-select-sm agt-dep-l1" name="<?= e($cfKey) ?>_l1"
                            ...
                            <select class="form-select form-select-sm agt-dep-l2" name="<?= e($cfKey) ?>_l2"
                                    data-field="<?= (int) $cf['id'] ?>"
                            ...
                            <?php if ($levels >= 3): ?>
                            <select class="form-select form-select-sm agt-dep-l3" name="<?= e($cfKey) ?>_l3"
                                    data-field="<?= (int) $cf['id'] ?>"
...
<?php if (!empty($customFields)): ?>
// Dependent field cascading dropdowns (agent ticket view)
(function() {
    document.querySelectorAll('.agt-dep-l1').forEach(function(sel) {
        sel.addEventListener('change', function() {
            var fid   = this.dataset.field;
            var l1Val = this.value;
            var opts  = JSON.parse(this.dataset.opts || '[]');
            var wrap  = document.getElementById('agt_dep_' + fid);
            var l2Sel = wrap ? wrap.querySelector('.agt-dep-l2') : null;
            var l3Sel = wrap ? wrap.querySelector('.agt-dep-l3') : null;
            if (l2Sel) {
                l2Sel.innerHTML = '<option value="">' + l2Sel.options[0].text + '</option>';
                if (l1Val) {
                    ...
                    l2Sel.style.display = children.length ? '' : 'none';
                } else { l2Sel.style.display = 'none'; }
            }
            if (l3Sel) { l3Sel.innerHTML = '<option value="">' + (l3Sel.options[0] ? l3Sel.options[0].text : '...') + '</option>'; l3Sel.style.display = 'none'; }
        });
    });
    document.querySelectorAll('.agt-dep-l2').forEach(function(sel) {
        sel.addEventListener('change', function() {
            var fid   = this.dataset.field;
            var l2Val = this.value;
            var l1Sel = document.getElementById('agt_dep_' + fid).querySelector('.agt-dep-l1');
            var opts  = JSON.parse(l1Sel ? l1Sel.dataset.opts || '[]' : '[]');
            var l3Sel = document.getElementById('agt_dep_' + fid).querySelector('.agt-dep-l3');
            if (l3Sel) {
                l3Sel.innerHTML = '<option value="">' + (l3Sel.options[0] ? l3Sel.options[0].text : '...') + '</option>';
                if (l2Val) {
                    ...
                    l3Sel.style.display = children.length ? '' : 'none';
                } else { l3Sel.style.display = 'none'; }
            }
        });
    });
})();
