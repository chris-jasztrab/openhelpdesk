...
- **AI Similar Past Tickets** — when an agent or admin opens a ticket, a sidebar panel surfaces the most relevant tickets from the whole archive (including resolved and closed) via a two-stage FULLTEXT-then-LLM rerank, each with a relevance note; result cached per ticket; a global master switch plus a per-ticket-type toggle (master overrides, with a live scope summary on the settings page), confidential types always excluded
