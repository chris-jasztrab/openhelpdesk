...
### Fixes
- **Killed the empty rows between system fields on the staff Create Ticket page.** The 2.29.1 grid change packed *Type / Status / Priority / Location / Due Date* into `col-md-6 col-lg-4`, but on `lg+` screens Priority still dropped to its own row with a tall gap above and below — because `$unifiedFields` interleaves system rows with **custom field rows**, and each custom field is wrapped in `<div class="col-12">` in [templates/pages/admin/tickets/create.php](templates/pages/admin/tickets/create.php). When no Type is selected (the default state), `filterFieldsByType()` hid only the inner `.custom-field-wrap` via `display:none` — but the surrounding `col-12` stayed in the DOM, contributed a `g-3` gutter row of empty space, and (worse) being full-width broke the bootstrap flex row so the next `col-lg-4` sibling started on a fresh visual line instead of slotting next to its predecessor. Two changes: (1) the wrapper now carries `class="col-12 custom-field-col" data-field-id="…"` so the JS has a stable handle on it; (2) `filterFieldsByType()` queries `.custom-field-col` and toggles `display` on the **column**, not the inner wrap, so a hidden custom field disappears from the layout entirely. Result: with no Type chosen, the Classification & Details card now renders as a clean two-row grid (*Type | Status | Priority* on row 1, *Location | Due Date | empty* on row 2) with no phantom whitespace; selecting a Type re-shows the relevant custom fields in their original order. Portal form is unaffected — it includes the partial directly without a column wrapper, so the inner-wrap toggle was already sufficient there.
