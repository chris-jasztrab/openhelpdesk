...
| Default Group | No | When set, only agents in this group are offered as assignees for tickets of this type. Required before enabling **Confidential**. When **Let AI route this to the best group** is on, this also doubles as the fallback queue used whenever AI can't confidently route — so make it your "No Wrong Door" team. |
| Confidential | No | Hard group-lock. Only members of the **Default Group** can view tickets of this type; admins outside the group must re-authenticate and all access is logged and notified to the group. Confidential types are automatically excluded from Location Ticket Visibility. Removing this flag from an existing type also requires re-authentication. Mutually exclusive with **Let AI route this to the best group** — confidential bodies are never sent to a third-party AI provider. |
| Let AI route this to the best group ("No Wrong Door") | No | Lets the AI classifier pick the destination group from every non-confidential group, using each group's `description` as the routing signal. Use this for a generic portal entry like *"I'm not sure who handles this"* — the patron submits and AI fans the ticket out to Branch IT, Cataloguing, Facilities, etc. based on the body. If AI's confidence is below the threshold (default 0.7) the ticket stays in the **Default Group**. Requires AI to be enabled at **Settings → AI Classification**. Routing quality depends on each group having a clear, specific description. See the [AI Classification docs](/admin/docs/ai#no-wrong-door) for the full audit trail and tuning guidance. |
| Show similar past tickets to agents | No | Checked by default. When an agent or admin opens a ticket of this type, a sidebar panel surfaces the most relevant tickets from the whole archive (including resolved and closed) with an AI relevance note. This per-type toggle only takes effect when the **global** master switch is on at **Settings → AI Classification** — that master switch overrides every per-type setting (off = hidden on all tickets; on = shown only on types whose toggle is also on). The settings page shows a live summary of which types have it on, off, or excluded. Blocked while **Confidential** is checked — confidential bodies are never sent to a provider. See the [AI Classification docs](/admin/docs/ai#similar-tickets). |
